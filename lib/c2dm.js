...
function C2DM(config) {
    if (config) {
        // legacy mode
        if ('legacymode' in config && config.legacymode) {
            ...
            if ('authorization' in config) {
                ...
                throw Error('No authorization key provided.')
            }
            ...
            this.c2dmOptions = {
                ...
                path: '/gcm/send',
                method: 'POST',
                headers: {'Content-Type': 'application/x-www-form-urlencoded', 'charset' : 'UTF-8', 'Authorization' : 'key='+this.authorization }
            };
        }
...
C2DM.prototype.send = function(packet, cb) {
    ...
    operation.attempt(function(currentAttempt) {
        ...
        var headers = self.c2dmOptions.headers;
